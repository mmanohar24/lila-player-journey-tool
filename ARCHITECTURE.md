...
With 93.3% of matches having exactly one participant, presenting all 796 as equivalent would bury the informative ones. The picker sorts richest-first with a participant-count badge and defaults to the richest match on load, while keeping the full list browsable. It also sorts by combat event count as a second, non-default option, since "richest" alone tends to surface bot-filled lobbies rather than a match with real fighting in it — see [INSIGHTS.md §1](./INSIGHTS.md#1-this-dataset-barely-has-any-humans-playing-together), the finding this second sort mode exists to act on. The sparsity itself is a real finding, not something to engineer out of view.
