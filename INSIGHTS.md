...
This is also why the match picker has a second sort mode — see [ARCHITECTURE.md's Match picker section](./ARCHITECTURE.md#match-picker) — since "richest" alone mostly surfaces bot-filled lobbies like the one above, not a match with real fighting in it.
...
- **Bots fight each other, and the README's own event table has no name for it.** It defines `BotKill`/`BotKilled` only from a human's point of view, but 183 `BotKill` and 297 `BotKilled` rows are filed under a bot's own journey: bot-on-bot combat the table just doesn't describe. Full writeup: [ARCHITECTURE.md's Event schema section](./ARCHITECTURE.md#event-schema).
