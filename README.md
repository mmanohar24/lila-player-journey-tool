...
in, so this works right away, with no extra setup needed.
...
Without it, the app falls back to Vercel's own production URL when deployed,
or `localhost:3000` when running locally.
...
- `ARCHITECTURE.md`: how data flows from parquet to screen, the coordinate
  mapping approach, and the tradeoffs behind it (one page, plus optional
  deeper-dive sections below it).
- `INSIGHTS.md`: three things the data actually showed once the tool was
  used on it for real.